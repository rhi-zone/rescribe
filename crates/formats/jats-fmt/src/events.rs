//! Streaming event types and iterator for JATS/XML documents.
//!
...
//! the input slice — it never materializes a `JatsDoc`. This is a true
...
/// A streaming iterator over JATS/XML events, produced by [`crate::events()`].
///
...
/// Build an owned event stream from an already-parsed [`crate::ast::JatsDoc`].
///
/// Used by the builder writer and by round-trip tests; not used by
/// [`crate::events()`] itself (which streams directly from bytes).
pub fn events_from_doc(doc: &crate::ast::JatsDoc) -> Vec<OwnedEvent> {
...
/// Reconstruct a `JatsDoc` from an event stream.
///
...
pub fn collect_doc(events: impl IntoIterator<Item = OwnedEvent>) -> crate::ast::JatsDoc {
