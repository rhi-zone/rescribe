//! JATS/XML emitter: converts a `JatsDoc` AST back to bytes using
...
/// Emit a `JatsDoc` as XML bytes.
pub(crate) fn emit(doc: &JatsDoc) -> Vec<u8> {
...
/// capture a JATS-specific sub-structure (`<article-meta>` front-matter
/// fields with no dedicated IR modeling) verbatim for raw preservation by a
/// downstream consumer.
...
mod tests {
    ...
    fn emits_element_with_attrs_and_text() {
        ...
        assert!(xml.contains("<p role=\"note\">Hello &amp; &lt;world&gt;</p>"));
    }

    #[test]
    fn roundtrips_through_parse() {
        let input = br#"<?xml version="1.0"?><article><title>T</title><p>Body</p></article>"#;
