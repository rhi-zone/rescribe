//! Oracle harness: compare textile-fmt parse output against Pandoc.
//!
//! These tests are `#[ignore]` by default. They require:
//!   - `~/git/pandoc/test/textile-reader.textile` (GPL corpus, never committed)
//!   - `pandoc` binary on PATH
//!
//! Run with:
//!   cargo test -q -p textile-fmt -- --ignored --nocapture
//!
...
/// Run `pandoc --from=textile --to=plain --quiet <file>` and return the plain
/// text output.  Returns `None` if pandoc is unavailable or fails.
fn pandoc_to_plain(pandoc: &Path, file: &Path) -> Option<String> {
    let out = Command::new(pandoc)
        .args(["--from", "textile", "--to", "plain", "--quiet"])
        .arg(file)
        .output()
        .ok()?;
...
fn words_from_inlines(inlines: &[Inline], out: &mut Vec<String>) {
    for inline in inlines {
        match inline {
            ...
            | Inline::Citation(children, _) => words_from_inlines(children, out),
            Inline::Link { children, .. } => words_from_inlines(children, out),
            Inline::GenericSpan { children, .. } => words_from_inlines(children, out),
            Inline::Code(s, _) => words_from_str(s, out),
            Inline::Acronym { text, title, .. } => {
                words_from_str(text, out);
                words_from_str(title, out);
            }
            Inline::Image { .. }
            | Inline::FootnoteRef { .. }
            | Inline::LineBreak(_)
...
fn words_from_cell(cell: &TableCell, out: &mut Vec<String>) {
...
fn words_from_blocks(blocks: &[Block], out: &mut Vec<String>) {
    for block in blocks {
        match block {
            ...
            Block::Blockquote { blocks: inner, .. } => words_from_blocks(inner, out),
            Block::List { items, .. } => {
                for item in items {
            ...
            Block::Table { rows, .. } => {
                for row in rows {
                    for cell in &row.cells {
                        words_from_cell(cell, out);
                    }
                }
            }
            Block::FootnoteDef { inlines, .. } => words_from_inlines(inlines, out),
            Block::DefinitionList { items, .. } => {
                for (term, def) in items {
                    words_from_inlines(term, out);
                    words_from_inlines(def, out);
                }
            }
            Block::HorizontalRule { .. } | Block::Raw { .. } => {}
        }
    }
}

fn extract_words(doc: &TextileDoc) -> Vec<String> {
    let mut out = Vec::new();
...
/// Run the oracle harness against the Pandoc textile corpus files.
///
/// Requires:
///   - `~/git/pandoc/test/` with textile corpus files
///   - `pandoc` binary on PATH
///
...
#[test]
#[ignore]
fn pandoc_textile_corpus() {
    run_harness(CORPUS_FILES);
}

/// Smoke test: parse the built-in representative Textile sample without
