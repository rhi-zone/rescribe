//! Streaming TikiWiki writer — converts a stream of events to TikiWiki text.
//!
...
impl<W: Write> Writer<W> {
    ...
    /// Flush all buffered events as TikiWiki text and return the sink.
...
fn events_to_doc(events: Vec<OwnedEvent>) -> TikiwikiDoc {
...
enum Frame {
    ...
    WikiLink { page: String, inlines: Vec<Inline> },
}
...
impl DocBuilder {
    ...
    fn process(&mut self, event: OwnedEvent) {
        match event {
            ...
            OwnedEvent::EndTableCell => {
                ...
                {
                    cells.push(TableCell { inlines, span: Span::NONE });
                }
            }
            ...
            OwnedEvent::StartWikiLink { page } => {
                self.stack.push(Frame::WikiLink { page, inlines: vec![] });
            }
            OwnedEvent::EndWikiLink => {
                if let Some(Frame::WikiLink { page, inlines }) = self.stack.pop() {
                    self.push_inline(Inline::WikiLink { page, children: inlines, span: Span::NONE });
                }
            }
    ...
    fn push_inline(&mut self, inline: Inline) {
        match self.stack.last_mut() {
            ...
            Some(Frame::WikiLink { inlines, .. }) => inlines.push(inline),
