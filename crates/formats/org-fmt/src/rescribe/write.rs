...
//!
//! Maps rescribe `Document`/`Node` to this crate's AST, then calls
//! `OrgDoc`'s `rescribe_format_api::Emit::emit()` to produce the final bytes.

use crate::{Block, DefinitionItem, Inline, ListItem, ListItemContent, OrgDoc, Span, TableRow};
use rescribe_core::{
...
mod tests {
    use super::*;
    use crate::rescribe::builder::org;
