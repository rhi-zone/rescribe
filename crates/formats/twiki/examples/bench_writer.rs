...
fn main() {
    ...
    // region — each iteration gets its own freshly-built `Vec<Event>`.
    ...
    println!(
        "twiki Writer: {ITERS} iters x {SECTIONS} sections, {:.3}s total, {:.0} ns/iter, {:.2} MB/s",
        elapsed.as_secs_f64(),
