...
//! Streaming TWiki writer — converts a stream of events to TWiki text.
//!
//! # Example
//! ```no_run
//! use twiki::writer::Writer;
...
/// [`finish`](Writer::finish) to flush TWiki text to the underlying sink.
...
impl<W: Write> Writer<W> {
    ...
    /// Flush all buffered events as TWiki text and return the sink.
...
fn events_to_doc(events: Vec<OwnedEvent>) -> TwikiDoc {
...
enum Frame {
    ...
    TableCell { inlines: Vec<Inline>, is_header: bool },
...
impl DocBuilder {
    ...
    fn process(&mut self, event: OwnedEvent) {
        match event {
            ...
            OwnedEvent::StartListItem => {
                self.stack.push(Frame::ListItem { inlines: vec![], children: vec![] });
            }
            OwnedEvent::EndListItem => {
                ...
                {
                    items.push(ListItem { inlines, children, span: Span::NONE });
                }
            }
            ...
            OwnedEvent::StartTableCell { is_header } => {
                self.stack.push(Frame::TableCell { inlines: vec![], is_header });
            }
            OwnedEvent::EndTableCell => {
                if let Some(Frame::TableCell { inlines, is_header }) = self.stack.pop() {
                    if let Some(Frame::TableRow { cells }) = self.stack.last_mut() {
                        cells.push(TableCell { inlines, is_header, span: Span::NONE });
                    }
                }
            }
            ...
            OwnedEvent::EndDefinitionTerm => {
                if let Some(Frame::DefinitionTerm { inlines }) = self.stack.pop() {
                    if let Some(Frame::DefinitionList { items }) = self.stack.last_mut() {
                        items.push(DefinitionItem { term: inlines, desc: vec![], span: Span::NONE });
                    }
                }
            }
            ...
            OwnedEvent::CodeBlock { content } => {
                self.push_block(Block::CodeBlock { content: content.into_owned(), span: Span::NONE });
            }
            OwnedEvent::HorizontalRule => {
                self.push_block(Block::HorizontalRule { span: Span::NONE });
            }
            OwnedEvent::RawBlock { content } => {
                self.push_block(Block::RawBlock { content: content.into_owned(), span: Span::NONE });
            }
            ...
            OwnedEvent::StartBoldItalic => {
                self.stack.push(Frame::BoldItalic { inlines: vec![] });
            }
            OwnedEvent::EndBoldItalic => {
                if let Some(Frame::BoldItalic { inlines }) = self.stack.pop() {
                    self.push_inline(Inline::BoldItalic(inlines, Span::NONE));
                }
            }
            ...
            OwnedEvent::StartBoldCode => {
                self.stack.push(Frame::BoldCode { inlines: vec![] });
            }
            OwnedEvent::EndBoldCode => {
                if let Some(Frame::BoldCode { inlines }) = self.stack.pop() {
                    self.push_inline(Inline::BoldCode(inlines, Span::NONE));
                }
            }
            ...
            OwnedEvent::Image { url, alt } => {
                self.push_inline(Inline::Image { url, alt, span: Span::NONE });
            }
            OwnedEvent::RawInline { content } => {
                self.push_inline(Inline::RawInline { content, span: Span::NONE });
            }
            OwnedEvent::WikiWord { word } => {
                self.push_inline(Inline::WikiWord { word, span: Span::NONE });
            }
        }
    }
    ...
    fn push_inline(&mut self, inline: Inline) {
        match self.stack.last_mut() {
            ...
            Some(Frame::BoldItalic { inlines }) => inlines.push(inline),
            ...
            Some(Frame::BoldCode { inlines }) => inlines.push(inline),
