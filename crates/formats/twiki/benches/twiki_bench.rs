...
fn parse(input: &str) -> (TwikiDoc, Vec<rescribe_format_api::Diagnostic>) {
    TwikiDoc::parse(input.as_bytes())
}

fn build(doc: &TwikiDoc) -> String {
    String::from_utf8(doc.emit()).expect("twiki emit output is UTF-8")
}
