...
use crate::{RisDoc, RisEntry, Span};
use rescribe_core::{
...
/// `crate::rescribe::read`'s `entry_to_node` any more, which now emits
/// `bibliography_entry`).
...
/// Write a `bibliography_entry` node (see `crate::rescribe::read`'s
...
fn emit_bibliography_entry(node: &Node, ctx: &mut EmitContext) {
    let mut entry = RisEntry::new(node.props.get_str("ris:type").unwrap_or("GEN"));
...
fn emit_bibtex_entry(node: &Node, ctx: &mut EmitContext) {
    ...
    let ris_type = crate::bibtex_type_to_ris(bibtex_type);
    let mut entry = RisEntry::new(ris_type);
...
fn emit_bibtex_fields(node: &Node, entry: &mut RisEntry) {
    for (key, value) in node.props.iter() {
        if let Some(field) = key.strip_prefix("bibtex:")
            && let rescribe_core::PropValue::String(s) = value
            && let Some(ris_tag) = crate::bibtex_field_to_ris(field)
        {
...
fn emit_citation_entry(node: &Node, ctx: &mut EmitContext) {
    ...
    let ris_type = crate::csl_type_to_ris(csl_type);
    let mut entry = RisEntry::new(ris_type);
