//! Chunk-driven (batch) Jira wiki markup parser.
//!
...
//! # Example — AST style
//! ```no_run
//! use jira_fmt::batch::BatchParser;
//!
...
//! ```no_run
//! use jira_fmt::batch::{StreamingParser, Handler};
...
enum BlockState {
    ...
    Accumulating,
    /// Inside a `{code}`, `{quote}`, `{panel}`, or `{noformat}` block.
    InDelimitedBlock { end_marker: String },
}
...
impl<H: Handler> StreamingParser<H> {
    ...
    fn feed_line(&mut self, line: String) {
        ...
        let is_end_of_delimited: Option<bool> =
            if let BlockState::InDelimitedBlock { ref end_marker } = self.state {
                Some(line.trim() == end_marker.as_str())
            } else {
                None
            };

        if let Some(is_end) = is_end_of_delimited {
        ...
        // Delimited block start
        let trimmed = line.trim();
        if trimmed.starts_with("{code") && trimmed.ends_with('}') {
            if !self.block_lines.is_empty() {
                self.emit_block();
            }
...
/// Chunk-driven Jira parser that delivers events to a callback on finish.
