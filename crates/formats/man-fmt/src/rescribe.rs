//! AST↔`rescribe::Document` translation for man pages.
//!
//! This module only translates between [`ManDoc`](crate::ManDoc) and
//! rescribe's `Document` IR — no roff/troff tokenizing/parsing/emitting
...
mod read {
    ...
    pub fn parse(input: &str) -> Result<ConversionResult<Document>, ParseError> {
        let (man_doc, _diags) = crate::parse::parse(input);
...
mod write {
    use crate::{Block, Emit, Inline, ManDoc, Span};
