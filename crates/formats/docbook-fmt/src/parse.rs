...
pub(crate) fn parse(input: &[u8]) -> (DocBookDoc, Vec<Diagnostic>) {
...
<article><para>Made by &company;.</para></article>"#,
        );
        assert!(diags.is_empty(), "diagnostics: {diags:?}");
        let root = doc.root().unwrap();
    ...
    fn resolves_entity_via_standard_table_without_any_doctype() {
        ...
        let (doc, diags) = parse("<para>caf\u{e9} or caf&eacute;</para>".as_bytes());
        assert!(diags.is_empty(), "diagnostics: {diags:?}");
        let root = doc.root().unwrap();
    ...
    fn still_preserves_entity_unresolvable_by_either_layer() {
        ...
        let (doc, diags) = parse(
            br#"<!DOCTYPE article [ <!ENTITY company "Acme, Inc."> ]>
<article><para>a &undeclared; b</para></article>"#,
        );
        assert!(diags.is_empty(), "diagnostics: {diags:?}");
        let root = doc.root().unwrap();
        let para = root
            ...
            .iter()
            .find_map(|n| match n {
                Node::Element { name, children, .. } if name == "para" => Some(children),
                _ => None,
            })
            .unwrap();
        assert!(
