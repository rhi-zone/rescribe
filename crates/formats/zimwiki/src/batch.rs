//! Chunk-driven (batch) ZimWiki parser.
//!
//! # Example — AST style
//! ```no_run
//! use zimwiki::batch::BatchParser;
//!
...
//! ```no_run
//! use zimwiki::batch::{StreamingParser, Handler};
//! use zimwiki::OwnedEvent;
//!
...
use crate::ast::{Diagnostic, ZimwikiDoc};
use crate::events::OwnedEvent;

/// Chunk-driven ZimWiki parser that returns the full AST on finish.
#[derive(Default)]
...
impl BatchParser {
    ...
    pub fn finish(self) -> (ZimwikiDoc, Vec<Diagnostic>) {
        let s = String::from_utf8_lossy(&self.buf);
        crate::parse::parse(&s)
    }
}

/// Handler trait for streaming ZimWiki events.
///
...
/// Chunked streaming ZimWiki parser that delivers events to a [`Handler`].
...
impl<H: Handler> StreamingParser<H> {
    ...
    fn feed_line(&mut self, line: String) {
        // Inside a verbatim block
        if matches!(self.state, BlockState::InVerbatimBlock) {
        ...
        if line.trim_start().starts_with("'''") {
            ...
            self.state = BlockState::InVerbatimBlock;
...
/// Chunk-driven ZimWiki parser that delivers events to a callback on finish.
