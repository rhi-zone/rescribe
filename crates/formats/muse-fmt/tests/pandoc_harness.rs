//! Oracle harness: compare muse-fmt parse output against Pandoc.
//!
//! These tests are `#[ignore]` by default. They require:
//!   - `~/git/pandoc/test/muse-reader.muse` (GPL corpus, never committed)
//!   - `pandoc` binary on PATH
//!
//! Run with:
//!   cargo test -q -p muse-fmt -- --ignored --nocapture
//!
...
/// Run `pandoc --from=muse --to=plain --quiet <file>` and return the plain
...
fn words_from_table_row(row: &TableRow, out: &mut Vec<String>) {
...
fn words_from_blocks(blocks: &[Block], out: &mut Vec<String>) {
    for block in blocks {
        match block {
            ...
            | Block::SrcBlock { content, .. } => words_from_str(content, out),
            Block::Blockquote { children, .. }
            | Block::Verse { children, .. }
            | Block::CenteredBlock { children, .. }
            | Block::RightBlock { children, .. } => words_from_blocks(children, out),
            Block::List { items, .. } => {
                for item in items {
            ...
            Block::DefinitionList { items, .. } => {
                for (term, def) in items {
                    ...
                    words_from_blocks(def, out);
                }
            }
            Block::Table { rows, .. } => {
                for row in rows {
                    words_from_table_row(row, out);
                }
            }
            Block::FootnoteDef { content, .. } => words_from_inlines(content, out),
            Block::Comment { .. } | Block::HorizontalRule { .. } => {}
        }
    }
}

fn extract_words(doc: &MuseDoc) -> Vec<String> {
    let mut out = Vec::new();
    // Include metadata fields
    if let Some(title) = &doc.title {
        words_from_str(title, &mut out);
    }
    if let Some(author) = &doc.author {
        words_from_str(author, &mut out);
    }
...
/// Run the oracle harness against the Pandoc muse corpus file.
///
/// Requires:
///   - `~/git/pandoc/test/muse-reader.muse`
///   - `pandoc` binary on PATH
///
...
#[test]
#[ignore]
fn pandoc_muse_corpus() {
    run_harness(CORPUS_FILES);
}

/// Smoke test: parse the built-in representative Muse sample without
