...
fn handle_event(
    ...
    match event {
        ...
        Event::StartFootnoteDef { label } => {
            ...
            inline_ctx.push(InlineFrame::FootnoteDef {
                label,
                inlines: Vec::new(),
            });
        }
        ...
        Event::EndBlockquote => {
            if let Some(BlockFrame::Blockquote {
                attribution,
                children,
            }) = block_stack.pop()
            {
        ...
        Event::EndFootnoteDef => {
            if let Some(InlineFrame::FootnoteDef { label, inlines }) = inline_ctx.pop() {
                let footnote = Inline::FootnoteDef {
                    label,
                    children: inlines,
                    span: Span::NONE,
                };
        ...
        Event::StartStrong => {
            inline_ctx.push(InlineFrame::Strong {
                inlines: Vec::new(),
            });
        }
        Event::StartEmphasis => {
            inline_ctx.push(InlineFrame::Emphasis {
                inlines: Vec::new(),
            });
        }
        ...
        Event::StartHighlight => {
            inline_ctx.push(InlineFrame::Highlight {
                inlines: Vec::new(),
            });
        }
        ...
        Event::StartSmallCaps => {
            inline_ctx.push(InlineFrame::SmallCaps {
                inlines: Vec::new(),
            });
        }
        Event::StartQuoted { quote_type } => {
            ...
            inline_ctx.push(InlineFrame::Quoted {
                quote_type: qt,
                inlines: Vec::new(),
            });
        }
        ...
        Event::EndEmphasis => {
            if let Some(InlineFrame::Emphasis { inlines }) = inline_ctx.pop() {
                push_inline(
                    block_stack,
                    inline_ctx,
                    Inline::Emphasis(inlines, Span::NONE),
                );
            }
        }
        ...
        Event::EndHighlight => {
            if let Some(InlineFrame::Highlight { inlines }) = inline_ctx.pop() {
                push_inline(
                    block_stack,
                    inline_ctx,
                    Inline::Highlight(inlines, Span::NONE),
                );
            }
        }
        Event::EndStrikeout => {
        ...
        Event::EndSmallCaps => {
            if let Some(InlineFrame::SmallCaps { inlines }) = inline_ctx.pop() {
                push_inline(
                    block_stack,
                    inline_ctx,
                    Inline::SmallCaps(inlines, Span::NONE),
                );
            }
        }
        Event::EndQuoted => {
            if let Some(InlineFrame::Quoted {
                quote_type,
                inlines,
            }) = inline_ctx.pop()
            {
...
mod tests {
    ...
    fn test_events_code_block() {
        ...
        assert!(evs.iter().any(
            |e| matches!(e, OwnedEvent::StartCodeBlock { language: Some(l) } if l == "python")
        ));
