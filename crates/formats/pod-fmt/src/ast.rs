...
impl Block {
    pub fn strip_spans(self) -> Self {
        match self {
            ...
            Block::Encoding { encoding, .. } => Block::Encoding {
                encoding,
                span: Span::NONE,
            },
        }
...
impl Inline {
    pub fn strip_spans(self) -> Self {
        match self {
            ...
            Inline::Bold(ch, _) => Inline::Bold(
                ch.into_iter().map(Inline::strip_spans).collect(),
                Span::NONE,
            ),
            Inline::Italic(ch, _) => Inline::Italic(
                ch.into_iter().map(Inline::strip_spans).collect(),
                Span::NONE,
            ),
            Inline::Underline(ch, _) => Inline::Underline(
                ch.into_iter().map(Inline::strip_spans).collect(),
                Span::NONE,
            ),
            ...
            Inline::Filename(ch, _) => Inline::Filename(
                ch.into_iter().map(Inline::strip_spans).collect(),
                Span::NONE,
            ),
            Inline::NonBreaking(ch, _) => Inline::NonBreaking(
                ch.into_iter().map(Inline::strip_spans).collect(),
                Span::NONE,
            ),
