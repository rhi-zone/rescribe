...
pub fn parse_with_options(
    ...
    let (doc, _diagnostics) = PodDoc::parse(input.as_bytes());
...
fn convert_block(block: Block) -> Node {
    match block {
        ...
        Block::Encoding { encoding, .. } => Node::new("encoding").prop("encoding", encoding),
    }
}

fn convert_inlines(inlines: Vec<Inline>) -> Vec<Node> {
    inlines.into_iter().map(convert_inline).collect()
}

fn convert_inline(inline: Inline) -> Node {
    match inline {
        Inline::Text(s, _) => Node::new(node::TEXT).prop(prop::CONTENT, s),
        ...
        Inline::Code(content, _) => Node::new(node::CODE).prop(prop::CONTENT, content),
        ...
        Inline::NonBreaking(children, _) => Node::new(node::SPAN)
            .prop("pod:format", "nonbreaking")
            .children(convert_inlines(children)),

        Inline::IndexEntry(entry, _) => Node::new(node::RAW_INLINE).prop(prop::CONTENT, entry),

        Inline::Null(_) => Node::new(node::RAW_INLINE)
            .prop(prop::FORMAT, "pod")
            .prop(prop::CONTENT, "Z<>"),

        Inline::Entity(s, _) => Node::new(node::TEXT).prop(prop::CONTENT, s),
    }
}
