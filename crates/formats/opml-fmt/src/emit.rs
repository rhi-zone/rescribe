...
pub(crate) fn emit(doc: &OpmlDoc) -> Vec<u8> {
    ...
    // A `<head>` with no fields at all is indistinguishable, once parsed,
    // from no `<head>` element being present in the source (see
    // `Head::is_empty`'s doc comment on `ast.rs`) — so this picks the
    // canonical choice of omitting it, matching `events()`/`events_from_doc`
    // (see events.rs) rather than always synthesizing an empty `<head></head>`.
