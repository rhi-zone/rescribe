...
mod tests {
    ...
    /// shared `AtomicUsize`): `cargo test` runs this file's other tests on
    /// multiple threads sharing this one process-wide allocator, and a
    /// shared counter would let an unrelated concurrently-running test's
    /// allocations inflate this test's peak — found via a real flake under
    /// full-workspace `cargo test -q` in this batch's `pod-fmt` sibling
    /// (peak ratio 407x from cross-thread noise, not a real regression;
    /// passed cleanly under `--test-threads=1`). Thread-local counters make
