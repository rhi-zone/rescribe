//! Corpus test - try parsing all DOCX files in the NapierOne corpus.
...
#[test]
#[ignore] // Run with `cargo test -p ooxml-wml -- --ignored`
fn test_napierone_docx_corpus() {
    let corpus_path = Path::new("/home/me/git/ooxml/corpora/napierone/DOCX");
    ...
    for entry in fs::read_dir(corpus_path).unwrap() {
            ...
            .is_some_and(|e| e.eq_ignore_ascii_case("docx"))
        {
            ...
            match Document::open(&path) {
                Ok(doc) => {
                    // Try to access the body to exercise the parser
                    let _body = doc.body();
                    success += 1;
                }
                Err(e) => {
