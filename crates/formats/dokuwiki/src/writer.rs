...
mod tests {
    ...
    //
    // `CURRENT`/`PEAK` are signed and use wrapping arithmetic: the
    // allocator intercepts *every* allocation in the test binary, including
    // background/concurrently-running unrelated tests, so a dealloc that
    // was allocated before a test's reset point can legitimately drive the
    // running total negative. Signed + wrapping avoids a debug-mode
    // overflow panic from that cross-test noise; `TEST_LOCK` serializes the
    // two tests below against each other (the dominant, controllable source
    // of contamination) so at least they don't corrupt one another.
    #[allow(unsafe_code)]
    mod alloc_probe {
            ...
            unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
                ...
                if !ptr.is_null() {
                    ...
                    let cur = CURRENT
                        ...
                        .wrapping_add(layout.size() as i64);
    ...
    fn test_writer_peak_memory_bounded() {
        ...
        // `PEAK` is signed (see `alloc_probe`'s doc comment on cross-test
        // allocator noise); a peak reading at or below zero just means no
        // net-positive live allocation was observed while this test held
        // `TEST_LOCK`, which trivially satisfies both bounds below.
        let peak = PEAK.load(Ordering::Relaxed).max(0) as usize;
