//! Streaming MediaWiki writer -- converts a stream of events to MediaWiki text.
//!
//! # Example
//! ```no_run
...
/// [`finish`](Writer::finish) to flush MediaWiki text to the underlying sink.
...
impl<W: Write> Writer<W> {
    ...
    /// Flush all buffered events as MediaWiki text and return the sink.
...
fn events_to_doc(events: Vec<OwnedEvent>) -> MediawikiDoc {
...
impl DocBuilder {
    ...
    fn process(&mut self, event: OwnedEvent) {
        match event {
            ...
            OwnedEvent::StartListItem => {
                self.stack.push(Frame::ListItem { blocks: vec![], inlines: vec![] });
            }
            OwnedEvent::EndListItem => {
                if let Some(Frame::ListItem { mut blocks, inlines }) = self.stack.pop() {
                    if !inlines.is_empty() {
            ...
            OwnedEvent::PreBlock { content } => {
                self.push_block(Block::PreBlock {
                    content: content.into_owned(),
                    span: Span::NONE,
                });
            }
            ...
            OwnedEvent::StartTable { caption } => {
                self.stack.push(Frame::Table { rows: vec![], caption });
            }
            OwnedEvent::EndTable => {
                if let Some(Frame::Table { rows, caption }) = self.stack.pop() {
                    self.push_block(Block::Table { rows, caption, span: Span::NONE });
                }
            }
            ...
            OwnedEvent::EndBold => {
                if let Some(Frame::Bold { inlines }) = self.stack.pop() {
                    self.push_inline(Inline::Bold(inlines));
                }
            }
            ...
            OwnedEvent::EndItalic => {
                if let Some(Frame::Italic { inlines }) = self.stack.pop() {
                    self.push_inline(Inline::Italic(inlines));
                }
            }
            ...
            OwnedEvent::EndLink => {
                if let Some(Frame::Link { url, inlines }) = self.stack.pop() {
                    let text = inlines
                        ...
                        .collect::<String>();
                    self.push_inline(Inline::Link { url, text });
                }
            }
            ...
            OwnedEvent::FootnoteRef { label, content } => {
                self.push_inline(Inline::FootnoteRef { label, content });
            }
            OwnedEvent::MathInline { source } => {
                self.push_inline(Inline::MathInline { source });
            }
            OwnedEvent::Template { content } => {
                self.push_inline(Inline::Template { content });
            }
            OwnedEvent::Nowiki { content } => {
                self.push_inline(Inline::Nowiki { content });
            }
        }
    }
    ...
    fn finish(mut self) -> MediawikiDoc {
        let blocks = match self.stack.pop() {
            Some(Frame::Document { blocks }) => blocks,
            _ => vec![],
        };
        MediawikiDoc { blocks, span: Span::NONE }
    }
}
