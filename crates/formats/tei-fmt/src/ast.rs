//! AST types for TEI (and generic XML) documents.
...
/// A TEI (or generic XML) document.
///
/// This AST is deliberately format-agnostic within the "TEI family" — it
/// represents any well-formed XML document as a tree of elements, text,
/// comments, CDATA sections, and processing instructions. TEI-specific
/// meaning (which element names mean what — `<div>`, `<hi rend="…">`,
/// `<ref target="…">`, `<teiHeader>`, etc.) lives in the rescribe adapter
/// layer, not here — the same way `jats-fmt`'s and `docbook-fmt`'s
/// `Node::Element` don't know which tags are "semantic."
#[derive(Clone, Debug, PartialEq, Default)]
pub struct TeiDoc {
    /// The `<?xml ... ?>` declaration, if the source had one.
    pub xml_decl: Option<XmlDecl>,
    /// Top-level nodes: usually a single root `Element` (`<TEI>`), plus any
    /// leading or trailing comments / processing instructions / doctype.
    pub nodes: Vec<Node>,
}

impl TeiDoc {
    pub fn strip_spans(&self) -> TeiDoc {
        TeiDoc {
...
pub enum Node {
    ...
    /// `<!DOCTYPE ...>` declaration, captured verbatim (TEI documents
    /// commonly declare their DTD/schema here, e.g. `-//TEI P5//DTD Main
    /// DTD Driver File//EN`).
...
impl Node {
    ...
    /// A specific attribute value by name. Attribute names are preserved
    /// exactly as written in the source (including any namespace prefix,
    /// e.g. `xml:id` or `xml:lang`) — no prefix-stripping is performed, so
    /// callers must match on the full attribute name they expect.
