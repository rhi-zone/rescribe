//! TEI/XML parser using `quick-xml`'s pull-based `Reader`.
//!
...
//!
//! Attribute keys are kept exactly as written (including any namespace
//! prefix, e.g. `xml:id`, `xml:lang`) rather than stripped to their local
//! name — TEI leans heavily on `xml:id`/`xml:lang`, and matching against the
//! literal prefixed name (as the adapter layer does) only works if the
//! prefix survives parsing.
...
/// Parse a TEI/XML document from bytes (assumed UTF-8).
///
...
pub(crate) fn parse(input: &[u8]) -> (TeiDoc, Vec<Diagnostic>) {
  ...
  <text>
    <body>
      <p>Hello, world!</p>
        ...
        assert_eq!(root.name(), Some("TEI"));
    }

    #[test]
    fn parses_attributes_and_entities() {
        let (doc, _) = parse(br#"<ref target="a&amp;b">text</ref>"#);
        let root = doc.root().unwrap();
        assert_eq!(root.get_attr("target"), Some("a&b"));
        assert_eq!(root.text_content(), "text");
    }

    #[test]
    fn preserves_namespaced_attribute_names() {
        // xml:id / xml:lang are pervasive in TEI; the prefix must survive
        // parsing verbatim (no local-name stripping) or the adapter layer's
        // attribute matching against "xml:id" can never succeed.
        let (doc, _) = parse(br#"<div xml:id="d1" xml:lang="en">text</div>"#);
        let root = doc.root().unwrap();
        assert_eq!(root.get_attr("xml:id"), Some("d1"));
        assert_eq!(root.get_attr("xml:lang"), Some("en"));
    }
    ...
    fn resolves_entity_declared_in_internal_subset() {
        let (doc, diags) = parse(
            br#"<!DOCTYPE TEI [ <!ENTITY company "Acme, Inc."> ]>
<TEI><p>Made by &company;.</p></TEI>"#,
        );
        assert!(diags.is_empty(), "diagnostics: {diags:?}");
        let root = doc.root().unwrap();
...
<TEI><p>a &undeclared; b</p></TEI>"#,
    ...
    fn recovers_from_truncated_input() {
        let (doc, diags) = parse(b"<TEI><text>unterminated");
        assert!(!diags.is_empty());
