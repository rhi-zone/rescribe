//! Chunk-driven (batch) TEI/XML parser with **true incremental**
//! event delivery.
//!
...
//! # Example — AST style
//! ```
//! use tei_fmt::batch::BatchParser;
//!
//! let mut p = BatchParser::new();
//! p.feed(b"<TEI><text><body><p>Hi</p>");
//! p.feed(b"</body></text></TEI>");
...
//! ```
//! use tei_fmt::batch::{StreamingParser, Handler};
//! use tei_fmt::OwnedEvent;
//!
//! let mut events = Vec::new();
//! let mut p = StreamingParser::new(|ev: OwnedEvent| events.push(ev));
//! p.feed(b"<TEI><p>Hello</p>");
//! p.feed(b"</TEI>");
//! p.finish();
...
use crate::ast::{Diagnostic, Severity, Span, TeiDoc};
use crate::events::OwnedEvent;

/// Chunk-driven TEI/XML parser that returns the full AST on finish.
///
...
impl BatchParser {
    ...
    pub fn finish(self) -> (TeiDoc, Vec<Diagnostic>) {
        crate::parse::parse(&self.buf)
    }
}

/// Handler trait for streaming TEI/XML events — the shared
...
/// Chunked streaming TEI/XML parser that delivers events to a
