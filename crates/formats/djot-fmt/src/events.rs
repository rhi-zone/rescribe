...
pub(crate) fn collect_blocks_from_event_iter<I: Iterator<Item = OwnedEvent>>(
    iter: I,
) -> Vec<Block> {
...
fn handle_event(
    ...
    match event {
        ...
        Event::StartCodeBlock {
            ...
            block_stack.push(BlockFrame::CodeBlock {
                language,
                content: String::new(),
                attr: make_attr(id, classes, kv),
            });
        ...
        Event::StartDiv {
            ...
            block_stack.push(BlockFrame::Div {
                class,
                blocks: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        ...
        Event::StartTable => {
            ...
            {
                if let Some(BlockFrame::TablePendingCaption(cap)) = block_stack.pop() {
                    Some(cap)
                } else {
                    None
                }
            ...
            block_stack.push(BlockFrame::Table {
                caption,
                rows: Vec::new(),
            });
        }
        ...
        Event::StartFootnoteDef { label } => {
            block_stack.push(BlockFrame::FootnoteDef {
                label,
                blocks: Vec::new(),
            });
        }
        ...
        Event::EndHeading => {
            if let Some(BlockFrame::Heading {
                level,
                inlines,
                attr,
            }) = block_stack.pop()
        ...
        Event::EndCodeBlock => {
            if let Some(BlockFrame::CodeBlock {
                language,
                content,
                attr,
            }) = block_stack.pop()
        ...
        Event::HardBreak => {
            push_inline_to_ctx(
                Inline::HardBreak { span: Span::NONE },
                block_stack,
                inline_stack,
            );
        }
        ...
        Event::StartEmphasis { id, classes, kv } => {
            inline_stack.push(InlineFrame::Emphasis {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartStrong { id, classes, kv } => {
            inline_stack.push(InlineFrame::Strong {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartDelete { id, classes, kv } => {
            inline_stack.push(InlineFrame::Delete {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartInsert { id, classes, kv } => {
            inline_stack.push(InlineFrame::Insert {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartHighlight { id, classes, kv } => {
            inline_stack.push(InlineFrame::Highlight {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartSubscript { id, classes, kv } => {
            inline_stack.push(InlineFrame::Subscript {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        Event::StartSuperscript { id, classes, kv } => {
            inline_stack.push(InlineFrame::Superscript {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
        ...
        Event::StartImage {
            ...
            inline_stack.push(InlineFrame::Image {
                url,
                title,
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
        ...
        Event::StartSpan { id, classes, kv } => {
            inline_stack.push(InlineFrame::Span {
                inlines: Vec::new(),
                attr: make_attr(id, classes, kv),
            });
        }
...
mod tests {
    ...
    fn test_events_code_block() {
        ...
        assert!(evs.iter().any(
            |e| matches!(e, OwnedEvent::StartCodeBlock { language: Some(l), .. } if l == "rust")
        ));
    ...
    fn test_events_table_caption() {
        ...
        let cap_pos = evs
            .iter()
            .position(|e| matches!(e, OwnedEvent::TableCaption(_)));
        ...
        assert!(
            cap_pos.unwrap() < start_pos.unwrap(),
            "TableCaption must precede StartTable"
        );
