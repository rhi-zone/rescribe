//! Chunk-driven (batch) VimWiki parser.
//!
...
//! lines, or delimited by `{{{`...`}}}` markers). Memory usage is
...
//! # Example -- AST style
//! ```no_run
//! use vimwiki_fmt::batch::BatchParser;
//!
...
//! ```no_run
//! use vimwiki_fmt::batch::{StreamingParser, Handler};
...
use crate::ast::{Diagnostic, VimwikiDoc};
use crate::events::OwnedEvent;

/// Chunk-driven VimWiki parser that returns the full AST on finish.
#[derive(Default)]
...
impl BatchParser {
    ...
    pub fn finish(self) -> (VimwikiDoc, Vec<Diagnostic>) {
        let s = String::from_utf8_lossy(&self.buf);
        crate::parse::parse(&s)
    }
}

/// Handler trait for streaming VimWiki events.
///
...
/// Chunked streaming VimWiki parser that delivers events to a [`Handler`].
///
...
impl<H: Handler> StreamingParser<H> {
    ...
    fn feed_line(&mut self, line: String) {
        // -- Inside a preformatted block --------------------------------------
        if matches!(self.state, BlockState::InPreformatted) {
        ...
        if trimmed.starts_with("{{{") && !crate::parse::is_inline_preformatted_pub(&line) {
            if !self.block_lines.is_empty() {
                self.emit_block();
            }
            self.state = BlockState::InPreformatted;
            self.block_lines.push(line);
            return;
        }
...
/// Chunk-driven VimWiki parser that delivers events to a callback on finish.
