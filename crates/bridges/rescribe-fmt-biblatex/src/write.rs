//! BibLaTeX writer half of `rescribe-fmt-biblatex`.
//!
...
//! `Bibliography::to_biblatex_string()` (the same crate this crate's `read`
//! module and `rescribe-fmt-bibtex` use to parse). This adapter's only job
//! is building a `biblatex::Entry` from the rescribe IR shapes it accepts —
//! it does not hand-roll escaping or field/entry syntax itself.
