...
fn convert_block(block: pod_fmt::Block) -> Node {
    match block {
        ...
        pod_fmt::Block::RawBlock {
            format, content, ..
        } => Node::new(node::RAW_BLOCK)
            .prop(prop::FORMAT, format)
            .prop(prop::CONTENT, content),

        pod_fmt::Block::ForBlock {
            format, content, ..
        } => Node::new("for_block")
            .prop(prop::FORMAT, format)
            .prop(prop::CONTENT, content),

        pod_fmt::Block::Encoding { encoding, .. } => {
            Node::new("encoding").prop("encoding", encoding)
        }
    }
}
...
fn convert_inline(inline: pod_fmt::Inline) -> Node {
    match inline {
        ...
        pod_fmt::Inline::NonBreaking(children, _) => Node::new(node::SPAN)
            .prop("pod:format", "nonbreaking")
            .children(convert_inlines(children)),

        pod_fmt::Inline::IndexEntry(entry, _) => {
            Node::new(node::RAW_INLINE).prop(prop::CONTENT, entry)
        }
