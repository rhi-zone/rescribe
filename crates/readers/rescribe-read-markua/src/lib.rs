...
pub fn parse_with_options(
    ...
    let markua_doc = markua::parse(input).map_err(|e| ParseError::Invalid(e.to_string()))?;
...
fn convert_inline(inline: &markua::Inline) -> Node {
    match inline {
        markua::Inline::Text(s) => Node::new(node::TEXT).prop(prop::CONTENT, s.as_str()),

        markua::Inline::Strong(children) => {
            Node::new(node::STRONG).children(convert_inlines(children))
        }

        markua::Inline::Emphasis(children) => {
            Node::new(node::EMPHASIS).children(convert_inlines(children))
        }

        markua::Inline::Strikethrough(children) => {
            Node::new(node::STRIKEOUT).children(convert_inlines(children))
        }

        markua::Inline::Code(s) => Node::new(node::CODE).prop(prop::CONTENT, s.as_str()),
        ...
        markua::Inline::LineBreak => Node::new(node::LINE_BREAK),

        markua::Inline::SoftBreak => Node::new(node::SOFT_BREAK),
    }
}
