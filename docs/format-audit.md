...
| fb2 | KnownFailure, narrowed further (2026-08-04): the originally-tracked `Metadata`-drop bug and three further `events()`-vs-AST-projection bugs were fixed 2026-08-03 (see prior audit history). `adv-malformed` — previously mis-scoped as "genuine architectural divergence... would require buffering/deferred emission" — is now fixed: that assessment was checked directly against quick_xml's tokenization and found false. An unclosed `<FictionBook ...>` start tag swallows the following `<body>` text, so quick_xml never emits a real `Start("body")` token at all; a `<section>` immediately after it has no `<body>`/`<section>` ancestor on `SemanticState`'s already-tracked `ParseState` stack, and that ancestor-validity check needs zero lookahead — it's knowable from the stack *before* the `<section>` Start token is even dispatched. Fixed via a `suppress_depth` counter that suppresses an orphaned `<section>` (and everything nested inside it) from the instant its Start tag is seen, mirroring `parse()`'s own `StackItem::Section` finalize arm, which silently drops a `Section` it can't attach to a `Body`/`Section` ancestor. This surfaced one dependent gap: `events()`/`StreamingParser` never synthesized a matching `EndFictionBook` when the token stream terminates early with `FictionBook` still open — fixed by extracting `close_item` from `handle_end`'s per-element closing match into a new `finalize_open_elements`, called at every point the stream can terminate for good (Eof, a fatal `Err`, `StreamingParser`'s own tag-mismatch branch), mirroring docbook-fmt/jats-fmt/tei-fmt's `finalize()` (2026-08-03) — again no lookahead, just draining the already-bounded stack. Confirmed via `fb2_events_equals_ast_projection_over_all_fixtures` and `fb2_streaming_writer_byte_identical_to_builder_over_all_fixtures` no longer diverging on `adv-malformed`, and `fb2_streaming_parser_matches_events_and_is_incremental` (adversarial chunking) continuing to pass. One fixture still diverges, left open: `annotation` is a genuine unimplemented feature (the book's own `<description><title-info><annotation>` sub-content, with `<p>`/`<poem>`/`<cite>`/`<subtitle>`/`<table>`/empty-line children, isn't modeled at all in `events.rs`'s description parsing — a real second content-model builder, not a quick fix; independently re-scoped 2026-08-04,
scoping investigation only, no code change — confirmed via `parse.rs`'s own
`Stanza`/`VerseLine`/`Cite`/`TextAuthor` finalization needing multi-level parent/grandparent
context to route correctly (~40+ interdependent match arms), which `events.rs` has no
production equivalent for; `<p>` and `<table>` alone would be small, but a partial fix
covering only those would still silently leak `<poem>`/`<cite>`/`<subtitle>` inside an
annotation, so it wasn't attempted). A related bug also fixed 2026-08-03: a description-context self-closing `<image>` (e.g. inside `<title-info><coverpage>`) used to leak out as a stray top-level `Event::Image`; now a no-op, matching `parse()` (which also doesn't populate `TitleInfo.coverpage` — a separate, pre-existing, `parse()`-side gap affecting both APIs equally). A `parse.rs`-only bug (not `events()`) was fixed alongside: `<custom-info>` text content was stolen by an unconditional `flush_text()` call before `handle_end` could read it, so `parse()`'s own AST used to report empty content | Wired (fixed 2026-07-31): `StreamingParser` rewritten from buffer-all-fed-bytes-then-parse-in-`finish()` to a true incremental drain reusing the same `SemanticState` dispatch machine `events()`/`EventIter` already used internally — the same "rebuild a `Reader` over just the unconsumed tail, `Err` means wait for more bytes" technique `docbook-fmt` pioneered, plus a hand-tracked `open_stack` replicating quick-xml's `check_end_names` validation (also mirroring docbook-fmt) so malformed-XML fatal-stop behavior matches `events()` exactly. Confirmed byte-for-byte equal to `events()` under adversarial chunking (whole/single-byte/chunks-of-N/mid-UTF-8-char) over every fb2 fixture; peak memory measured flat (~3.09KB) across a 10x input-size increase (200 vs 2000 synthetic sections), not O(full input) | KnownFailure, narrowed further (2026-08-04): the writer itself is genuinely incremental and fed by `events()`, so the `adv-malformed` fix above fixed this downstream too, for free — no writer-side change needed. The one remaining `events()`-side gap (`annotation`) still reproduces downstream, for the same underlying reason documented in the `events()` column |
