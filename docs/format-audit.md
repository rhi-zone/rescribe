...
| djot | Wired (independent recursive-descent vs. frame-stack impls) | KnownFailure: 6/79 fixtures, 4 distinct bugs (nested-div flag-not-counter, cross-block link-ref resolution, block-attr flushed before fence, def-list split) | KnownFailure: architecturally buffer-then-emit, not incremental (`Event::LinkDef` now exists and round-trips correctly — the prior `LinkDef`-drop gap is closed) |
...
| org | Wired (independent of `parse()`; dependency runs the other way) | KnownFailure: 3/89 fixtures, 3 distinct bugs (nested `#+BEGIN_QUOTE` has no depth counter, `#+NAME:` flushed before its block, indented list code-block misread as top-level) | Wired (`Event::Metadata { key, value }` added — `#+TITLE:`/`#+AUTHOR:`/keyword lines now round-trip through `events()`/`StreamingParser`/the streaming `Writer`; still buffer-then-emit, not incremental) |
...
| texinfo | Wired (includes `Event::Title` for `@settitle`) | KnownFailure: architecturally hollow — `feed()` buffers into a `Vec<u8>`, all parsing happens in `finish()` (crate's own module doc says `O(full input)`) | KnownFailure: same hollow buffer-then-emit pattern (content, including `@settitle` via `Event::Title`, round-trips correctly; the gap is purely incrementality) |
