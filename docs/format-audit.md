...
### Clean (39) — adapter is a thin AST↔IR translator on both sides
...
jats (`jats-fmt`), tei (`tei-fmt`), opml (`opml-fmt`), html (`html-fmt`), rtf (`rtf-fmt`),
ris (`ris`), csv (`csv-fmt`), tsv (`tsv-fmt`), native (`native`).
