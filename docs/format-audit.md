...
| markua | 5 | 5 | U | hand | – | – |
| fountain | 5 | 5 | U | hand | – | – |
| typst | 3 | 3 | U | typst-syntax (typst-fmt) | fuzz (targets exist, compile-clean; not yet run — no `cargo-fuzz` binary in this session's sandbox; standalone-validated ~300k pseudo-random samples instead, see typst-fmt's fuzz target module docs) | fuzz (same caveat); construct gaps remain (see `fixtures/typst/COVERAGE.md`) |
| texinfo | 5 | 5 | U | hand | – | – |
| bbcode | 5 | 5 | U | hand | – | – |
| pod | 5 | 5 | U | hand | – | – |
| haddock | 5 | 5 | U | hand | – | – |
...
for these two cases any more. **Found but not fixed at the time, flagged in TODO.md:** (a)
several known (non-`Unknown`) BibLaTeX-only entry types still collapse to `misc` when
`rescribe-write-bibtex` targets classic BibTeX, with no warning — **resolved 2026-08-05 (later
same day):** confirmed against `biblatex` 0.11's own `EntryType::to_bibtex()` that exactly
`Periodical`/`Reference`/`MvReference`/`Patent`/`Online`/`Software`/`Dataset`/`Set`/`XData`
collapse to `Misc` (as opposed to a lossless rename like `Report -> TechReport`); concluded
splicing the raw type name back (as done for `Unknown`) would be *wrong* here, not just
unnecessary — `@software{...}` in output requested as classic BibTeX would silently vanish under
most real `.bst` styles (they don't define handling for it), whereas `@misc` renders generically
under every style, so `to_bibtex()`'s downgrade is dialect-correct and only the missing warning
was the bug. `rescribe-write-bibtex` now emits a `FidelityWarning` (`FeatureLost`, `Minor`) for
this case via a shared `entry_type_and_diagnostics` helper; `rescribe-write-biblatex` needed no
change (`to_biblatex()` doesn't downgrade any BibLaTeX-only type). (b) `rescribe-read-biblatex`'s
...
repo currently rounds a document through both — **still open**). New fixtures:
`fixtures/bibtex/rare-uncertain-date`, `fixtures/bibtex/rare-custom-type`,
`fixtures/biblatex/rare-custom-type`.
...
Nine of the ten below (all but `plaintext`, untouched this pass) got their first
multi-construct `fixtures/writers/{format}/` coverage 2026-08-04 (slide/frame boundary or
structural unit, paragraph, list, code block, bold/italic emphasis — see the dated entry
above for exact fixtures and two silent-image-drop bugs found in revealjs/slidy/s5/
dzslides/ms/icml, tracked in TODO.md, not fixed this pass).

| Format | W | CC | Library | W-next |
|--------|---|----|---------|--------|
| beamer | 4 | – | hand | fixtures done this pass; still no fidelity-warning mechanism on any dropped construct |
| revealjs | 4 | – | hand | fix silent `Image`-in-paragraph drop (`src/lib.rs:141`-area `emit_inline_node`, see TODO.md) |
| slidy | 4 | – | hand | add `IMAGE` handling (currently absent everywhere, see TODO.md) |
| s5 | 4 | – | hand | add `IMAGE` handling (currently absent everywhere, see TODO.md) |
| dzslides | 4 | – | hand | add `IMAGE` handling (currently absent everywhere, see TODO.md) |
| slideous | 4 | – | hand | fixtures done this pass; image support already present and correct |
| context | 4 | – | hand | fixtures done this pass (heading/paragraph/list/code/emphasis/table/image all confirmed working) |
| ms | 4 | – | hand | add `IMAGE` handling or an explicit documented non-goal + fidelity warning (see TODO.md) |
| icml | 4 | – | hand | add `IMAGE` handling (ICML/InCopy natively supports embedded images, see TODO.md) |
| chunkedhtml | 4 | – | hand | fixtures done this pass; multi-file chunk+index generation confirmed working |
...
| html | Wired (2026-08 rework, replacing the earlier `events_from_doc(&parse(input).0)` tree-walk implementation): both driven by `html_fmt::sink::IncrementalSink`, a custom `html5ever::TreeSink` that emits events as the tokenizer/tree-builder produce them — a bounded correction-event pattern (`NodeReparented`/`ChildrenReparented`/`NodeDetached`, keyed on stable `NodeId`s assigned at node-creation time) handles HTML5's retroactive tree-construction operations (foster parenting, adoption agency), verified bounded to nodes still on the stack of open elements via a html5ever 0.36.1 `TreeSink` call-site trace. `crates/rescribe-fixtures/tests/streaming_apis/html.rs`'s `html_events_incremental_matches_parse_over_all_fixtures` confirms `collect_doc(events())` reconstructs the exact same tree as `parse()` over the full fixture suite (100+ fixtures); html-fmt's own crate tests additionally cover two hand-written adversarial cases the fixture suite doesn't exercise (`<table><div>foo</div></table>` for foster parenting, `<b>1<div>2</b>3</div>` for a genuine adoption-agency "furthest block" restructuring, the one case that actually fires the three correction events). One verified, documented gap, not a tree-correctness issue: `Event::EndElement` is not guaranteed for every `StartElement` — html5ever's adoption-agency "no furthest block" branch and its inner-loop stack cleanup remove a node from the open-elements stack via `Vec::truncate`/`Vec::remove` without ever calling `TreeSink::pop`, reachable even via plain foster parenting (`<table><div>foo</div></table>` never emits `EndElement` for `div` or `table`, despite both being explicitly closed in the source) — tree structure is unaffected since it comes from parent/child links, not `EndElement`; see `Event::EndElement`'s doc comment. Known regression, not closed here: the streaming `Writer` (below) now buffers every event into an in-memory tree until `finish()`, even for correction-free streams, because a byte-stream writer can't un-write bytes a later correction might move — see `writer.rs`'s module doc | Wired (2026-08 rework): `feed()` drives the same `IncrementalSink` per chunk, dispatching every event the chunk makes available before returning; `html_streaming_parser_feed_is_incremental` confirms events reach the handler before `finish()`. `html_streaming_parser_matches_events_under_adversarial_chunking` confirms `StreamingParser` reconstructs (via `collect_doc`) the same tree as bulk `events()` under every adversarial chunking including a mid-UTF-8-character split — a structural check, not `Vec<OwnedEvent>` equality, since genuine incremental delivery legitimately splits text runs at finer granularity when fed smaller chunks (`single_byte` chunking splits a merged text run into `Text`+`TextAppended` events that bulk `events()`'s coarser internal chunking would have merged into one `Text`) | Wired, byte-identical to `emit()` over all fixtures — unchanged, independent code from the reader-side rework; now pays a buffering cost it didn't used to (see the `events()` cell) |
...
| fb2 | Wired (fixed 2026-08-04): the last open gap — the book's own `<description><title-info><annotation>` (same content model as `<section>`: `Para`/`EmptyLine`/`Poem`/`Cite`/`Subtitle`/`Table`, per `AnnotationContent` in `ast.rs`) was silently discarded by `events()`/`EventIter`, leaving `TitleInfo.annotation` permanently `None` while `parse()`'s AST modeled it correctly. Fixed via a dedicated `ann_stack: Vec<AnnoItem>` content-builder on `SemanticState`, separate from the top-level `stack`: `AnnoItem` mirrors the subset of `parse.rs`'s `StackItem` reachable from `Annotation`'s content model (`Paragraph`/`Subtitle`/`Poem`/`PoemTitle`/`Stanza`/`VerseLine`/`Cite`/`Epigraph`/`TextAuthor`/`Table`/`TableRow`/`TableCell`/`InlineWrapper`/`Link`/`FootnoteRef`), assembling owned AST values bottom-up exactly like `parse.rs`'s `Parser` does — not streamed as top-level events, since the whole thing must fold into a single `Event::Metadata` rather than emit incrementally the way identical-looking `<poem>`/`<cite>` content in a `<section>` does. `handle_start`/`handle_empty`/`handle_end`/`flush_inline_text`/`finalize_open_elements` were all given an `ann_stack` priority check that routes to new `handle_ann_*`/`close_ann_item`/`ann_push_block_content`/`ann_in_inline_context`/`ann_push_text_to_inline_context`/`ann_push_inline` methods mirroring `parse.rs`'s equivalents scoped to `ann_stack`. Four new fixtures added (`fixtures/fb2/annotation-{poem,cite,table,subtitle}`) alongside the existing `annotation` fixture (a bare `<p>`), covering every `AnnotationContent` variant plus `Poem`'s own nested `Epigraph`. Confirmed via `fb2_events_equals_ast_projection_over_all_fixtures` and `fb2_streaming_writer_byte_identical_to_builder_over_all_fixtures` both passing over the full 61-fixture suite; `fb2_streaming_parser_matches_events_and_is_incremental` (adversarial chunking) continues to pass. Both `KnownFailure` entries removed from `streaming_harness::KNOWN_FAILURES`; `events`/`streaming_writer` promoted to Wired. **2026-08-04 (later): the separate, previously-deferred *section-level* `<section><annotation>` gap (`Section.annotation`, distinct from `TitleInfo.annotation`) is now also closed.** Unlike title-info's annotation, `parse.rs` already treats `Section.annotation` as an ordinary stack frame (its content streams through the same `push_block_content` dispatch section content uses), so no `ann_stack`-style buffering was needed here — the only missing piece was a way to scope that already-streaming content so a consumer can tell it apart from `Section.content`. Added two new public `Event` variants, `StartAnnotation { id }`/`EndAnnotation`, structurally identical in role to the pre-existing `StartEpigraph`/`EndEpigraph` pair that already scopes `Section.epigraph`/`Poem.epigraph` content the same way. `handle_start`'s (non-title-info) `"annotation"` arm now emits `StartAnnotation` alongside pushing `ParseState::Annotation`; `close_item`'s `ParseState::Annotation` arm (previously a no-op) now emits `EndAnnotation`; `ParseState::Annotation` was also added to `in_inline_context`'s boundary-frame list (previously fell through transparently to whatever was beneath it on the stack — functionally equivalent in practice since a `Section`/`Body` frame is always further down, but now explicit and consistent with `Epigraph`/`Cite`/`Poem`/`Stanza`). `writer::Writer` gained matching `StartAnnotation`/`EndAnnotation` arms (writes `<annotation id="…">`/`</annotation>`, mirroring the `StartEpigraph`/`EndEpigraph` arms exactly) — the streaming writer needed no other changes since it already writes whatever content events arrive between the new Start/End pair. Five new fixtures added (`fixtures/fb2/section-annotation{,-poem,-cite,-table,-subtitle}`), mirroring the title-info annotation fixture set, each covering one `AnnotationContent` variant. Confirmed via `fb2_events_equals_ast_projection_over_all_fixtures` (its `fb2_ast_to_events` ground-truth projection in `crates/rescribe-fixtures/tests/streaming_apis.rs` gained a matching `fb2_annotation_events` helper, called from `fb2_section_events` for `section.annotation`) and `fb2_streaming_writer_byte_identical_to_builder_over_all_fixtures`, both green over the full (now 66-fixture) suite; `fb2_streaming_parser_matches_events_and_is_incremental` continues to pass. Deliberately not touched: the IR adapter (`rescribe-read-fb2`'s `convert_section`) still has no code path reading `Section.annotation` at all — it silently contributes nothing to the converted `Document`, with no fidelity warning — a distinct, still-open gap one layer up from `fb2-fmt` itself (out of scope for this fb2-fmt-internal fix; tracked in `TODO.md`) — **closed 2026-08-05**: `convert_section` now maps `Section.annotation` to a `blockquote` tagged `fb2:type = "annotation"`, placed before the section's own content (mirroring `convert_epigraph`'s `Section.epigraph` → `blockquote fb2:type=epigraph` treatment); `rescribe-write-fb2`'s `nodes_to_section` recognizes that tag and routes it back to `Section.annotation` (via a new `blockquote_to_annotation`) instead of falling through to the default `blockquote` → `Cite` path. The five `fixtures/fb2/section-annotation*` fixtures (previously only asserting the content didn't leak into the section's plain paragraphs) were updated to assert the new `blockquote fb2:type=annotation` node and its content shape | Wired (fixed 2026-07-31): `StreamingParser` rewritten from buffer-all-fed-bytes-then-parse-in-`finish()` to a true incremental drain reusing the same `SemanticState` dispatch machine `events()`/`EventIter` already used internally — the same "rebuild a `Reader` over just the unconsumed tail, `Err` means wait for more bytes" technique `docbook-fmt` pioneered, plus a hand-tracked `open_stack` replicating quick-xml's `check_end_names` validation (also mirroring docbook-fmt) so malformed-XML fatal-stop behavior matches `events()` exactly. Confirmed byte-for-byte equal to `events()` under adversarial chunking (whole/single-byte/chunks-of-N/mid-UTF-8-char) over every fb2 fixture; peak memory measured flat (~3.09KB) across a 10x input-size increase (200 vs 2000 synthetic sections), not O(full input) | Wired (fixed 2026-08-04): the writer itself is genuinely incremental and fed by `events()`, so the annotation fix above fixed this downstream too, for free — no writer-side change needed |
...
| commonmark / gfm / markdown (shared `commonmark-fmt`) | Wired (fixed 2026-08-04): the last remaining bug — `StartList` always reporting `tight: true`, never corrected — is closed. `events::Event` gained a new variant, `ListTightnessResolved { tight: bool }` (a breaking, semver-relevant change to commonmark-fmt's public API — see the crate's CHANGELOG.md). `EventIter` still emits `StartList { tight: true, .. }` optimistically (list tightness is a whole-list property, uniform across all items, that pulldown-cmark itself only reveals via whether item content arrives wrapped in real `Paragraph` tags — determining it can require seeing every item), but now tracks the signal (a real, direct-child `Paragraph` tag on any item, correctly excluding one nested inside a blockquote via a small per-item `quote_depth` counter) with O(nesting depth) state and, if the list turns out loose, emits `ListTightnessResolved { tight: false }` exactly once, immediately before the matching `EndList` — the one point in the stream the answer is always known, without ever buffering the whole list. Five other bugs found via this harness's `ast_to_events` projection check were fixed 2026-08-03: (1) image alt-text `Text` event was emitted before `StartImage` instead of between `StartImage`/`EndImage`, duplicating alt text in the output; (2) consecutive `Text` events from pulldown-cmark weren't coalesced the way `parse()`'s AST deliberately does; (3) tight list items' bare inline content (no `Paragraph` tags from pulldown-cmark) wasn't wrapped in a synthetic `StartParagraph`/`EndParagraph` pair the way `parse()`'s AST always does (`parse.rs`'s `flush_tight_inlines`); (4) table-head cells weren't wrapped in a synthetic `StartTableRow`/`EndTableRow` pair the way `parse()`'s AST always does, since pulldown-cmark never emits `Tag::TableRow` for the header row; (5) GFM email autolinks (`<user@example.com>`) weren't normalized to a `mailto:` URL the way `parse()`'s AST does. `Event::LinkDef` (reference-style `[label]: url` definitions, which pulldown-cmark's own event stream never surfaces) continues to round-trip through `events()`/`StreamingParser`/the streaming `Writer` | NotApplicable: buffering all input before parsing with pulldown-cmark is the sole documented CLAUDE.md-sanctioned exemption (pulldown-cmark requires the full `&str`) | Wired (fixed 2026-08-04): the writer is a genuine write-through streaming design, byte-identical to `build()` for every fixture including the three that exercise list tightness (`rare-loose-list`, `integration-loose-list-item`, `rare-tight-vs-loose`). It now reacts to `Event::ListTightnessResolved` by retroactively splicing in the blank-line separators it skipped on the (now-corrected) optimistic `tight: true` assumption — both between items and between two blank-line-separated blocks inside the same item — via a small parallel `Vec<Vec<usize>>` of skipped-separator byte offsets per open list (`Writer::list_correction_marks`), so it stays correct regardless of exactly where in the list the correction arrives, including the case where the item revealing the signal isn't the one whose own separator needed fixing |
| docx | Wired (fixed 2026-08-03): `events()` was dropping the `Text` event and reversing `EndRun`/`EndParagraph` order for the common `<w:p><w:r><w:t>` shape (no `<w:pPr>`). Root cause was two compounding bugs in `read_props`'s scan-ahead-for-props logic: `open()` pushed a container's own stack frame only *after* `build_start_event` returned, so a child container found during the parent's props scan (e.g. `<w:r>` found while scanning `<w:p>` for `<w:pPr>`) pushed its frame *before* the parent's, inverting `End`-tag pop order; and `queue()` overwrote the single pending-event slot instead of prepending, so a `Text` event queued by a deeply nested scan (`<w:p><w:r><w:t>`, three levels) was clobbered by the parent's own queued event. Fixed by pushing the frame before scanning and replacing the fixed 2-slot `pending` array with a `VecDeque` that `queue()` prepends to. See `crates/rescribe-fixtures/tests/streaming_apis.rs`'s `wml_events_reaches_and_correctly_orders_paragraph_text` | NotYetWired (crate-level: `ooxml_wml::batch::StreamingParser<H>` added 2026-08-08, driven by `ooxml_opc::StreamingParser`, forwarding only `word/document.xml`'s bytes through the same true-SAX `events()` machinery; 5 crate-level adversarial-chunking/no-panic tests pass in `crates/formats/ooxml-wml/tests/streaming_reader.rs`. Memory bound O(largest part + nesting depth), not O(largest token). Updated 2026-08-08 (same day follow-up): `ooxml-opc`'s `Event::Part` no longer buffers a part's full bytes before delivery (fixed — see TODO.md's "ooxml-fmt rework" entry); the remaining O(largest part) bound is now attributable to this crate's own `events()`, which takes a complete `&[u8]` slice and has no chunk-fed XML tokenizer — a narrower, still-open gap, not the old ooxml-opc-container-layer one. Still NotYetWired at this harness's own level: no fixture-driven check in `rescribe-fixtures` yet, same gap as pptx below) | NotYetWired |
| pptx | Wired (fixed 2026-08-03): `events()` could not reach any slide text at all. `ooxml-pml` is an independent implementation, not shared with `ooxml-wml`, so it needed its own port of the docx fix (same push-before-build/prepend-not-overwrite pattern) plus a separate reachability gap: `dispatch_start()` has no dedicated `PmlEvent` for `<p:sld>`/`<p:cSld>`/`<p:spTree>`/`<p:txBody>` — real `ppt/slideN.xml` root structure and shape text wrapper — so they fell into `skip_element()` and the entire shape/paragraph/run/text subtree was silently dropped (verified against `fixtures/pptx/slide/input.pptx`'s actual `slide1.xml`: without a transparent-wrapper path, `events()` on the real file produced zero shape events at all, not just missing text). Fixed via a `is_transparent_wrapper` descend-without-emitting path mirroring ooxml-wml's, applied to `read_xml_info`/`read_xml_info_or_props` *and* `read_shape_transform` (a `<p:sp>` scanning for `<p:spPr>`, which schema-valid content may omit entirely — confirmed via a fixture with `<p:txBody>` and no `<p:spPr>`). Fixing `read_shape_transform` this way surfaced a further pre-existing fragility, shared with `read_props` in both crates: any text node — even pretty-printed whitespace between sibling elements — ended the scan early; harmless for real Word/PowerPoint output (never pretty-printed) but broke a hand-formatted crate-level fixture, so scans now only stop on non-whitespace text. See `pml_events_reaches_slide_text`/`pml_events_reaches_slide_text_with_sppr_present` in the same test file | NotYetWired (crate-level: `ooxml_pml::batch::StreamingParser<H>` added 2026-08-08, driven by `ooxml_opc::StreamingParser`; slide **display order** resolved from `<p:sldIdLst>` + presentation-part relationships, not ZIP/filename order, reusing `presentation.rs`'s existing resolution logic. 6 crate-level tests pass (display-order-vs-write-order, display-order-vs-filename-order, AST cross-check, adversarial chunking, no-panic, missing-slide diagnostic). Still NotYetWired at this harness's own level: no fixture-driven check in `rescribe-fixtures` yet) | Wired (2026-08-08: `crates/rescribe-fixtures/tests/streaming_apis/ooxml_pml.rs`'s `pml_streaming_writer_preserves_multi_slide_text` drives `PmlWriter` through a multi-slide event sequence via `new_slide()`, unzips the resulting PPTX, and checks each `ppt/slides/slideN.xml` part's text landed on the right slide — same byte-identical-to-builder-is-impossible-for-a-full-zip-package reasoning as xlsx's precedent below. Shape geometry (EMU position/size, needs YAML+codegen regen) is untouched, out of scope) |
| xlsx | Wired | NotYetWired (crate-level: `ooxml_sml::batch::StreamingParser<H>` added 2026-08-08, driven by `ooxml_opc::StreamingParser`, running `xl/worksheets/sheetN.xml` parts through the existing true-SAX `events()` iterator; `xl/sharedStrings.xml` parsed into `Event::SharedStrings` independently of sheet-part order, cell values always carry the raw shared-string index rather than a two-pass resolve. See the "ooxml-fmt rework" TODO.md entry for the full memory-bound writeup. Still NotYetWired at this harness's own level: no fixture-driven check in `rescribe-fixtures` yet, same gap as docx/pptx above) | Wired |
...
| opml | Wired — opml-fmt is a new crate (2026-08-04), not an audit of a pre-existing implementation. Same well-nested-XML architecture as docbook/jats/tei but with a domain-typed AST/event vocabulary (`OpmlDoc`/`Head`/`Body`/`Outline`, `StartOutline`/`HeadField`/`EmptyOutline`/...) instead of a generic element tree, since OPML's grammar is small and fixed. Verified over all 26 fixtures; found and fixed one real defect during wiring: an empty `Head` is indistinguishable from "no `<head>` in the source" once parsed, so `events_from_doc`/`emit` used to unconditionally synthesize `<head></head>`, diverging from `events()` on a headless document (fixture `adv-minimal`) — fixed by skipping the Start/EndHead pair when `Head::is_empty()` in both places | Wired — verified over all 26 fixtures plus a hand-built provably-complete-prefix probe (same shape as jats's) | Wired — found and fixed one real defect: no `in_opml` tracking meant a fatal XML error mid-document (fixture `adv-malformed-xml`, an unclosed `<outline>`) never synthesized a closing `EndOpml`, while the AST-derived projection always appends one — fixed by adding `in_opml` (mirroring `in_head`/`in_body`) to both `EventIter::finalize` and `StreamingParser::close_out` |
| endnotexml | Wired — endnotexml-fmt is a new crate (2026-08-04), extracted from the pre-existing `rescribe-read-endnotexml`/`rescribe-write-endnotexml` (which called `quick_xml` directly). Larger/deeper grammar than opml (nested contributor role lists, multiple title variants, `<style>` markup runs), so the AST/event vocabulary is domain-typed at the container level (`Record`/`Contributors`/`Titles`/`Periodical`/`Urls`/`Dates` each get their own `Start*`/`End*` pair) but generic at the leaf level (`StartElement{name,attrs}`/inline-content/`EndElement`, keyed by exact source tag name) — see `ast.rs`/`events.rs` module docs. Verified over all 22 fixtures; two defects found and fixed before this row was written: (1) `emit()`/streaming `Writer` used a pretty-printing `quick_xml::Writer::new_with_indent`, which corrupts meaningful whitespace between `<style>` runs — switched both to non-indenting `Writer::new`, matching the pre-existing writer's identical choice; (2) `events_from_doc`'s per-record field order was an arbitrary struct order not matching real EndNote export order (observed from the fixture files directly: `ref-type`, `contributors`, `titles`, `dates`, then `volume`/`pages`/`urls`/etc., `rec-number`/`foreign-keys` last) — reordered to match; record field order is canonicalized on write for a source with a nonstandard order (same known limitation as opml's `Head`), content fidelity is otherwise exact | Wired — verified over all 22 fixtures plus a hand-built provably-complete-prefix probe | Wired — verified over all 22 fixtures byte-identical to `emit()`, plus a genuine-incremental-write probe |
| typst | Wired — typst-fmt is a new crate (2026-08-04), extracted from the pre-existing `rescribe-read-typst`/`rescribe-write-typst` (which called `typst_syntax::parse` and hand-emitted Typst markup directly). Unlike opml-fmt/endnotexml-fmt, `typst-syntax` has no native event/SAX mode at all, so `events()` is a cursor-based walk (`EventIter`'s explicit `Vec<Task>` work stack) over the already-parsed structure — a legitimate streaming-iteration API (real per-`next()` incremental work, not a pre-materialized `Vec<Event>`), honestly a post-parse walk, same non-independent-but-still-Wired shape as bbcode-fmt below. Two genuine bugs found and fixed while adding a native-AST roundtrip test: `Block::DefinitionList` was emitted wrapped in `#terms(...)`, which is not valid Typst syntax; and equation/unknown-function-call source text was extracted via `SyntaxNode::text()`, empty for any composite node — fixed via `.into_text()` | Wired — buffers all input until `finish()`, the second CLAUDE.md-sanctioned "buffer all input" exemption alongside commonmark-fmt's pulldown-cmark: `typst-syntax` has no chunk-fed from-scratch parse API (`reparse`/`Source::edit` are edit-based reparsing of an already-built tree, an editor use case, not applicable here) | Wired — verified byte-identical to `emit()` over a construct sample in the crate's own `writer.rs` test |
| odt (`odf-fmt`) | NotYetWired: `events()` is a genuine independent quick_xml scan of `content.xml` (not a `parse()`-then-walk fake — corrects a prior assessment), but eagerly buffers every event before the first `next()` call (not memory-bounded); a faithful `ast_to_events` projection spanning odt/ods/odp body shapes is substantial follow-up work | Wired (2026-08-06): `batch::StreamingParser<H>` now exists, backed by `zip-fmt`'s hand-rolled push-based ZIP `StreamingParser` (parsing local file headers directly, not `zip::ZipArchive`'s end-of-file central directory) instead of the plain `zip` crate odf-fmt previously depended on for every other API. `content.xml` — the dominant contributor to a real document's size — is fed into a new `content_stream::ContentDriver` token-by-token as bytes arrive, mirroring `docbook-fmt`'s "rebuild a `Reader` over the unconsumed tail, `Err`/ambiguous-`Eof` means wait for more bytes" technique (including the same `check_end_names=false`/`allow_unmatched_ends=true` requirement — a real bug caught during development: the fresh per-call `Reader` has no record of an enclosing element's opening tag, so `check_end_names` raised a spurious `UnmatchedEndTag` the instant it reached that element's own closing tag, silently producing zero body events for any document containing an `<office:automatic-styles>` block, i.e. nearly all of them). One documented, bounded exception: `office:automatic-styles`, `office:annotation`, `text:note-citation`, field elements, and `text:soft-page-break` are each buffered in full (via a generic depth-tracking pre-scan, `subtree_end`) before being handed to the existing, unmodified `parser.rs`/`events.rs` helpers that already consume them in one call — bounded by that one construct's size, not the whole document, the same class of exception as `zip-fmt`'s own `Store`+data-descriptor case. `mimetype`/`meta.xml`/`styles.xml`/embedded images are still buffered per-entry (not per-archive). Verified over all 67 odt fixtures, both whole-input and byte-at-a-time feeding, via `crates/rescribe-fixtures/tests/streaming_apis/odf.rs`'s new `odf_streaming_parser_event_multiset_matches_events_under_adversarial_chunking` — a *multiset* comparison, not exact-sequence: `StreamingParser` delivers entries in ZIP physical order, while `events()` reads named entries in a fixed logical order (mimetype, meta.xml, styles.xml, content.xml) via random access, so event order can legitimately differ for an archive some other producer wrote with a different physical layout (this crate's own writer output happens to match, since it writes in that same order) | Wired (fixed 2026-08-03): all remaining 12/66 fixture divergences behind the byte-identical-to-builder check are fixed. Diffing per-fixture emitted `content.xml` (not just the first divergence the harness check reports) found seven distinct root causes, not the single "OdfEvent vocabulary gaps" bucket the entry previously described: (1) note internals unmodeled — no event for `<text:note-citation>`/`<text:note-body>`, so a note's marker and entire body were dropped (`DocBuilder` even had a `NoteBody` frame nothing pushed); added `NoteCitation`/`StartNoteBody`/`EndNoteBody` (footnote, footnote-formatted, endnote); (2) empty-element inline leaves had no variants for `<text:soft-hyphen/>`, `<text:soft-page-break/>`, `<text:bookmark/>`/`-start` — the previously-undiagnosed "heading-only divergence" is this same bug, not a heading bug (`fixtures/odt/heading`'s `<text:h>` contains a `<text:bookmark/>`) (bookmark, heading, soft-hyphen); (3) `<office:annotation>` corrupted output rather than merely losing it — emitted `Unknown` and the scan then descended into its children, leaking `<dc:creator>` text into the enclosing paragraph while the comment itself vanished; added `Annotation`, flattened the same way `parse()` does (annotation); (4) `StartCell` carried neither `table:number-columns-spanned` nor `table:number-rows-spanned` nor the typed value, and self-closing `<table:covered-table-cell/>` was recognized only in a spreadsheet body (colspan-rowspan); (5) block-level `<draw:frame>` was structurally unreachable — `EndFrame` routed through `push_inline` only, which matches nothing at block level; `<draw:text-box>` also had no text-body handling (text-box, image-caption); (6) `quick_xml::Event::GeneralRef` (character/entity references) had no arm in `events.rs`, though `parser.rs` has one, so `&#160;` was silently dropped (non-breaking-space); (7) `OdfEvent::Unknown` carried no raw payload, making raw preservation impossible through the event path — now carries verbatim XML and consumes its subtree, which required `events.rs` to track the enclosing content model (a `<table:table>` under `<office:text>` is a table; one inside `<text:p>` is unmodeled by `parser::parse_inlines` and captured raw) (path-deeply-nested-table). Field elements (`text:date`, `text:page-number`, ...) were named in the old description but no odt fixture exercises them — an `OdfEvent::Field` variant was added anyway for parity with `parse()`'s `Inline::Field`, though it fixed none of the 12. Two real losslessness gaps found during this work are *not* fixed, both on the `parse()` side where the AST path and the event path lose equally (so neither is a streaming defect and neither shows up in this check) — see TODO.md: `ast::FrameContent` is an either/or enum giving `<draw:image>` priority over a sibling `<draw:text-box>`, so a frame holding both an image and a caption loses the caption entirely; and self-closing field elements (as opposed to `<text:date>...</text:date>`) are dropped by both paths |
| ansi | NotYetWired: no `ast_to_events`-projection *check* is wired for ansi. The AST-side gap that used to make one impossible is fixed (2026-08-04, alongside the `streaming_writer` fix below): `AnsiNode` gained `SetStyle { style, span }`/`ResetStyle { span }` variants, emitted unconditionally, one per source SGR escape group, mirroring `events()`'s `SetStyle`/`ResetStyle` exactly — `parse()`'s AST now has a 1:1 node for every `events()` style event, so a faithful projection is possible; nobody has written the projection function/fixture check itself yet. Separately, the spurious-`ResetStyle`-on-no-op-SGR bug found via the checks below is fixed (2026-07-30): `apply_sgr_event()` now returns whether it saw an explicit reset code, only that triggers `ResetStyle` | Wired (2026-08-03 follow-up): all four bugs originally found are fixed. `drain_complete` carries running style across drains, no longer inherits spurious-`ResetStyle`, coalesces adjacent `Text` events via a `pending_text` accumulator (all 2026-07-30), and — the fourth, previously left open — `find_safe_boundary` gained a second pass (`truncate_before_unclosed_osc8_hyperlink`) that scans for a complete OSC 8 *opening* sequence with no matching closer yet in the safe prefix and backs the boundary off to before it, deferring the whole open..close span to a later call instead of exposing an isolated opener to `EventIter`. See TODO.md | Wired (fixed 2026-08-04, in two stages). Stage 1 narrowly fixed the `adv-unknown-sgr` divergence via a `ResetStyle` node emitted only when the running style was already empty before the reset — but verifying beyond that one fixture found the fix was far too narrow: 24 of 46 ansi fixtures diverged between `build()` and the streaming `Writer`, for two reasons that turned out to be the same underlying gap named by this row's `events` cell (`bold`: `build()` deferred its trailing reset past a trailing `Newline` node, giving `"...Hello\n\x1b[0m"` for source `"...Hello\x1b[0m\n"`; `rare-bold-italic`: `build()` re-derived one merged `\x1b[1;3m` from the resulting style instead of the source's two separate `\x1b[1m`/`\x1b[3m` groups, which `events()`/`Writer` reproduce verbatim). Stage 2 fixed the root cause directly: `AnsiNode` gained `SetStyle { style, span }` alongside the existing `ResetStyle { span }`, both now emitted unconditionally, one node per source SGR escape group in source order — `parse.rs`'s `m` arm is now a byte-for-byte match of `events.rs`'s `parse_csi_event` `m` arm. `build()`'s `emit_node` transitions style at each `SetStyle`/`ResetStyle` node's own source position instead of only at Text/Hyperlink nodes, replaying the same one-escape-per-source-group shape `events()`/`Writer` already produce. One AST change covered both previously-named root causes — they were the same gap, not two separate ones. A third, unrelated, previously-undiscovered bug in `writer.rs`'s `transition_style`/`append_all_style_codes` (silently dropping `double_underline`/`rapid_blink`/`underline_color` — SGR 21/6/58 — entirely) was also fixed, needed to close fixture `double-underline`. `rescribe-read-ansi`'s adapter was updated to match the new AST shape (`SetStyle` nodes are dropped — their style is already captured on the next Text/Hyperlink node's own field; `ResetStyle` nodes are kept as a raw_inline `\x1b[0m` only when the adapter's own running-style tracking shows the reset was otherwise unobservable, reproducing the exact IR output as before for every existing fixture). Measured directly (`crates/formats/ansi-fmt/examples/divergence_check.rs`, all 46 fixtures): 24/46 diverging before, 0/46 after |
...
*(stale — superseded by the full inventory below; odt/fb2/docbook/jats/tei/opml/native now have crates.)*
...
`typst` moved to Clean 2026-08-04: `typst-fmt` extracted (see TODO.md); `rescribe-read-typst`
no longer calls `typst_syntax::parse` directly (moved into `typst-fmt::parse`) and
`rescribe-write-typst`'s 508-line hand-rolled emitter is gone, replaced by a thin
`typst_fmt::{Block,Inline}` <-> IR translator calling `typst_fmt::emit`. `rescribe-read-typst`'s
`eval` feature (the full `typst` compiler crate, HTML-based evaluation) is unchanged and
out of scope for this migration — it is a separate code path (`parse_evaluated`), not the
syntax-only reader this row was about.
...
latex, bibtex, biblatex, csl-json, pandoc-json, ipynb, typst (writer side), and the 11
output-only targets. (`opml`/`endnotexml` now have `opml-fmt`/`endnotexml-fmt`, both
2026-08-04 — this line is otherwise unaudited since 2026-07-28.)
...
`quick_xml` library). `html-fmt` used to be listed here too (buffer-all,
justified by HTML5 tree construction being able to rearrange already-seen
nodes) until the 2026-08 rework proved that justification unnecessary: a
custom `html5ever::TreeSink` (`html_fmt::sink::IncrementalSink`) now emits
events as the tokenizer/tree-builder produce them, handling the
rearrangement cases (foster parenting, adoption agency) via bounded
correction events instead of buffering — see the html row in the Cross-API
harness inventory below.
...
bound is **O(largest single part)**, not O(nesting depth).

**Update 2026-08-08 (same day, follow-up) — the `ooxml-opc` container-layer part of
that bound is now fixed.** `ooxml_opc::StreamingParser` no longer buffers each ZIP
entry's decompressed bytes in full before delivering it — see the TODO.md "ooxml-fmt
rework" entry's newest bullet. What remains open, and is narrower than before: this
crate's own `crate::events::events` takes a complete `&[u8]` slice (no chunk-fed XML
tokenizer exists in this crate), so `StreamingParser<H>` still accumulates a
worksheet part's bytes into one buffer before running `events()` over it — the
memory bound for `xl/worksheets/sheetN.xml` specifically is still O(largest single
part), but the bottleneck is now this crate's own XML layer, not `ooxml-opc`'s
container layer. Every other part this crate doesn't specially interpret no longer
costs a full buffered copy at the `ooxml-opc` layer before being classified.
Reaching true O(nesting depth) for XLSX's actual large-file case requires a
...
### Typst reader — construct coverage still incomplete
- 2026-08-04: `typst-fmt` extracted as a standalone crate (see the R:3/W:3 row above and
  TODO.md) — the adapter-layer CLAUDE.md violation this note originally flagged (parsing
  logic in the adapter) is resolved, but construct coverage is unchanged by that extraction.
  Many constructs remain unmodeled (see `fixtures/typst/COVERAGE.md`): heading levels 3–6,
  `#grid`, horizontal lines, `#bibliography`/`#outline`/`#pagebreak`, `@label`/`<label>`
  references, `#highlight`/`#overline`/text color/font size, math functions, `#set`/`#show`
  rules, document metadata via `#set document(...)`, `#include`.
- Pandoc harness word-coverage number (~5%, ref=552 words, ours=36) not re-run this session;
  stale, needs re-measurement against the new crate.
