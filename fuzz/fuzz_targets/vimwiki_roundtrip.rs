...
//! VimWiki roundtrip fuzz target.
//!
//! Generates arbitrary rescribe Documents with VimWiki-compatible constructs,
//! emits them to VimWiki text via rescribe-write-vimwiki, parses them back
//! via rescribe-read-vimwiki, and asserts that text content is preserved.
//!
//! Direction: arbitrary_rescribe_doc → emit → parse → assert text preserved
//!
//! VimWiki uses:
//! - `*bold*`, `_italic_`, `~~strikethrough~~` for inline markup
//! - `= Heading =` through `====== Heading ======` for headings
//! - `* ` / `1. ` for unordered/ordered list items
//! - `[[url|text]]` for links, `` `code` `` for inline code
//!
...
/// Sanitise text: strip characters that would be re-interpreted as VimWiki markup.
fn sanitise(s: &str) -> Option<String> {
    ...
    loop {
        ...
        // Strip leading ". " (vimwiki parser treats empty-prefix ". " as ordered list)
        ...
        // Strip single-letter ordered list markers: "a) " or "b) " etc.
        if out.len() >= 3 {
            let b = out.as_bytes();
...
fuzz_target!(|blocks: Vec<FuzzBlock>| {
    ...
    let Ok(emit_result) = rescribe_write_vimwiki::emit(&doc) else {
        return;
    };
    let _vimwiki_for_debug = std::str::from_utf8(&emit_result.value)
        .unwrap_or("")
        .to_string();

    let Ok(vimwiki_str) = std::str::from_utf8(&emit_result.value) else {
        return;
    };
    let Ok(parse_result) = rescribe_read_vimwiki::parse(vimwiki_str) else {
        return;
    };
    ...
    assert_eq!(
        text_before,
        text_after,
        "VimWiki roundtrip lost text content\n  before: {text_before:?}\n  after:  {text_after:?}\n  vimwiki: {_vimwiki_for_debug:?}"
    );
});
