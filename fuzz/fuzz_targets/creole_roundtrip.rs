...
//! Creole roundtrip fuzz target.
//!
//! Generates arbitrary rescribe Documents with Creole-compatible constructs,
//! emits them to Creole text via rescribe-write-creole, parses them back via
//! rescribe-read-creole, and asserts that text content is preserved.
//!
//! Direction: arbitrary_rescribe_doc → emit → parse → assert text preserved
//!
//! Creole uses:
//! - `**bold**`, `//italic//` for inline markup
//! - `= Heading =` through `====== Heading ======` for headings
//! - `*` / `#` for unordered/ordered list items
//! - `[[url|text]]` for links, `{{{inline code}}}` for code
//!
//! Characters stripped from text: all ASCII control chars, `*`, `/`, `=`,
//! `[`, `]`, `{`, `}`, `|`, `#`, `!`, `\\`, `<`, `>`.
//! Ordered list patterns (N. and N) ) stripped to avoid re-parse ambiguity.
...
/// Sanitise text: strip characters that would be re-interpreted as Creole markup.
fn sanitise(s: &str) -> Option<String> {
    ...
    loop {
        ...
        // Strip ordered list prefixes: "N. " or "N) "
        loop {
...
fuzz_target!(|blocks: Vec<FuzzBlock>| {
    ...
    let Ok(emit_result) = rescribe_write_creole::emit(&doc) else {
        return;
    };
    let _creole_for_debug = std::str::from_utf8(&emit_result.value)
        .unwrap_or("")
        .to_string();

    let Ok(creole_str) = std::str::from_utf8(&emit_result.value) else {
        return;
    };
    let Ok(parse_result) = rescribe_read_creole::parse(creole_str) else {
        return;
    };
    ...
    assert_eq!(
        text_before,
        text_after,
        "Creole roundtrip lost text content\n  before: {text_before:?}\n  after:  {text_after:?}\n  creole: {_creole_for_debug:?}"
    );
});
