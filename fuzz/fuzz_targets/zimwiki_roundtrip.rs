...
//! ZimWiki roundtrip fuzz target.
//!
//! Generates arbitrary rescribe Documents with ZimWiki-compatible constructs,
//! emits them to ZimWiki text via rescribe-write-zimwiki, parses them back
//! via rescribe-read-zimwiki, and asserts that text content is preserved.
//!
//! Direction: arbitrary_rescribe_doc → emit → parse → assert text preserved
//!
//! ZimWiki uses:
//! - `**bold**`, `//italic//`, `__underline__`, `~~strike~~` for inline markup
...
/// Sanitise text: strip characters that would be re-interpreted as ZimWiki markup.
fn sanitise(s: &str) -> Option<String> {
    ...
    loop {
        ...
        // Strip leading ". " (zimwiki parser may treat empty-prefix ". " as ordered list)
...
fuzz_target!(|blocks: Vec<FuzzBlock>| {
    ...
    let Ok(emit_result) = rescribe_write_zimwiki::emit(&doc) else {
        return;
    };
    let _zimwiki_for_debug = std::str::from_utf8(&emit_result.value)
        .unwrap_or("")
        .to_string();

    let Ok(zimwiki_str) = std::str::from_utf8(&emit_result.value) else {
        return;
    };
    let Ok(parse_result) = rescribe_read_zimwiki::parse(zimwiki_str) else {
        return;
    };
    ...
    assert_eq!(
        text_before,
        text_after,
        "ZimWiki roundtrip lost text content\n  before: {text_before:?}\n  after:  {text_after:?}\n  zimwiki: {_zimwiki_for_debug:?}"
    );
});
