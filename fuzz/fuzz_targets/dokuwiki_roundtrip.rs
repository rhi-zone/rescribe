...
//! DokuWiki roundtrip fuzz target.
//!
//! Generates arbitrary rescribe Documents with DokuWiki-compatible constructs,
//! emits them to DokuWiki text via rescribe-write-dokuwiki, parses them back
//! via rescribe-read-dokuwiki, and asserts that text content is preserved.
//!
//! Direction: arbitrary_rescribe_doc → emit → parse → assert text preserved
//!
//! DokuWiki uses:
//! - `**bold**`, `//italic//`, `__underline__` for inline markup
//! - `====== Heading ======` (inverted depth) for headings
//! - `*` / `-` for unordered/ordered list items (indented with spaces)
//! - `[[url|text]]` for links
//!
...
/// Sanitise text: strip characters that would be re-interpreted as DokuWiki markup.
...
fuzz_target!(|blocks: Vec<FuzzBlock>| {
    ...
    let Ok(emit_result) = rescribe_write_dokuwiki::emit(&doc) else {
        return;
    };
    let _dokuwiki_for_debug = std::str::from_utf8(&emit_result.value)
        .unwrap_or("")
        .to_string();

    let Ok(dokuwiki_str) = std::str::from_utf8(&emit_result.value) else {
        return;
    };
    let Ok(parse_result) = rescribe_read_dokuwiki::parse(dokuwiki_str) else {
        return;
    };
    ...
    assert_eq!(
        text_before,
        text_after,
        "DokuWiki roundtrip lost text content\n  before: {text_before:?}\n  after:  {text_after:?}\n  dokuwiki: {_dokuwiki_for_debug:?}"
    );
});
