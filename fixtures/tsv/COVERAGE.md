# TSV Fixture Coverage

A fixture suite is complete when all items below are checked.
See `fixtures/spec.md` for category definitions.

TSV (Tab-Separated Values) is a variant of CSV using tab (`\t`) as the field delimiter.
There is no formal RFC; the rescribe reader maps TSV to a `table` with `table_header` cells
for the first row and `table_cell` cells for data rows.

## Table structure

- [x] basic table (header + data rows) — `basic`
- [x] three-column table — `three-columns`
- [ ] single-column table — (missing)
...
- [ ] trailing tab (empty last field) — (missing)
...
- [ ] header and data rows with whitespace-only cells — (missing)
